...
protected:
    ...
    void flush()
    {
        ...
        // Fast path: previous byte was not 0xFF and none of the 4 output bytes is 0xFF.
        // This is the common case (~98.4% of flushes) — write 4 bytes directly without loop or FF-handling.
        if (!is_ff_written_)
        {
            ...
            if (UNLIKELY(b0 == jpeg_marker_start_byte || b1 == jpeg_marker_start_byte ||
    ...
    slow_path:
        // Slow path: handle FF byte-by-byte (original algorithm).
        for (int i{}; i < 4; ++i)
        {
